...
export class OTPService {
  ...
  async sendVerificationOTP(email) {
    ...
    // Fire-and-forget: don't let a slow or stalled SMTP connection (common
    // on some hosts' free tiers) block the HTTP response for 30+ seconds.
    // The OTP is already stored, so the user can hit "resend" if the email
    // itself is delayed or fails — we just log failures instead of hanging.
